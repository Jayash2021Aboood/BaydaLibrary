                        ...
                        <form action="" method="GET">
                            <div class="input-group">
                                <input class="form-control" id="search_term" name="search_term" type="text"
                                    placeholder="<?php echo lang('Search in authors ...'); ?>"
        ...
        <div class="row">
            <?php
                if(isset($_GET['search_term']) && !empty($_GET['search_term']))
                    $all = getAllAuthorsBySearch($_GET['search_term']);
                else
                    $all = getAllAuthorsBySearch("");
                if(!(count($all) > 0)) echo /*html*/'<div class="col text-center"> <h2 class="text-danger" >No Authors Found To Display. </h2></div>'; 
                else{ 
                    foreach($all as  $row)
                    {
            ?>
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <a class="btn btn-primary btn-sm"
                            href="author.php?id=<?php echo $row['id'] ?>"><?php echo lang('Read more'); ?> →</a>
                    </div>
                </div>
            </div>
            <?php }} ?>
